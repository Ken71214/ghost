#!/usr/bin/env bash
# ghost-commit.sh — run claude, detect changes, stage, commit
...
cmd_commit() {
  ...
  local prompt=""
  local model="${GHOST_DEFAULT_MODEL}"
  local dry_run=0
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      *)
        ...
        echo "usage: ghost commit -m \"prompt\" [--model MODEL] [--dry-run]" >&2
        exit 1
        ;;
    esac
  done
